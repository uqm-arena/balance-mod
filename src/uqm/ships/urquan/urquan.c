...
#define WEAPON_OFFSET 34
#define SIDE_OFFSET 23
#define MISSILE_SPEED 80
#define MISSILE_LIFE 21
...
#define FIGHTER_LIFE 800
#define FIGHTER_LAUNCH_TIME 6
#define FIGHTER_PURSUIT_TIME 264
...
// Turret
#define AUXILIARY_ENERGY_COST 2
#define AUXILIARY_WAIT 21
#define TURRET_OFFSET 9
#define TURRET_MISSILE_OFFSET 4
#define TURRET_MISSILE_SPEED 80
#define TURRET_MISSILE_LIFE 8
...
initialize_fusion (ELEMENT *ShipPtr, HELEMENT FusionArray[])
{
	...
	MissileBlock.pixoffs = WEAPON_OFFSET;
	...
	if (FusionArray[0])
	{
		...
		// Add some of the Dreadnought's velocity to its projectiles
...
spawn_fighter_laser (ELEMENT *ElementPtr)
{
			...
			hObject; hObject = hNextObject)
	{
		...
		if (!elementsOfSamePlayer (ObjectPtr, ShipPtr)
				...
				&& ObjectPtr->life_span == NORMAL_LIFE)
		{
			SIZE delta_x, delta_y;
			long dist;

			delta_x = ObjectPtr->next.location.x - FighterPtr->next.location.x;
			delta_y = ObjectPtr->next.location.y - FighterPtr->next.location.y;
		...
		else if (!elementsOfSamePlayer (ObjectPtr, ShipPtr)
				&& ObjectPtr->state_flags & PLAYER_SHIP
				&& ObjectPtr->life_span > NORMAL_LIFE)
			isShielded = TRUE;
	...
	if (hBestObject)
	{
		...
		FighterPtr->thrust_wait += ((BYTE)TFB_Random () & FIGHTER_WEAPON_WAIT);
	/* Add some cooldown when aiming at a shielded enemy so fighters don't all
	   fire on the same frame when the shield comes down */
		...
		if (Enroute)
		{
			...
			if (hTarget)
			{
				...
				if (ElementPtr->thrust_wait > 0)
				{
					...
					// 2) Is the fighter behind the enemy's forward arc?
...
fighter_collision (ELEMENT *ElementPtr0, POINT *pPt0, ELEMENT *ElementPtr1, POINT *pPt1)
{
	...
	else if (ElementPtr0->pParent != ElementPtr1->pParent)
	{
		...
		// Drop a small explosion when a fighter is destroyed. Disabled for now.
		/* if (hBlastElement)
		{
			ELEMENT *BlastElementPtr;

			LockElement (hBlastElement, &BlastElementPtr);
			BlastElementPtr->current.location = ElementPtr0->current.location;
	...
	if (ElementPtr->hTarget)
	{
		...
		if (TargetPtr->hit_points)
		{
			...
			if (delta_facing > 0 && !OBJECT_CLOAKED(TargetPtr))
			{
				...
				// Homing projectile may turn a second time on the same frame if there's a need for it
				delta_facing = NORMALIZE_FACING (ANGLE_TO_FACING (ARCTAN (delta_x, delta_y)) - facing);
				if (delta_facing > 0)
				{
					if (delta_facing < ANGLE_TO_FACING (HALF_CIRCLE))
...
#include "../chenjesu/chenjesu.h"
// Need to see a preprocess function from this ship
...
initialize_turret (ELEMENT *ElementPtr)
{
	...
	for (hObject = GetPredElement (ElementPtr); hObject; hObject = hNextObject)
	{
		LockElement (hObject, &ObjectPtr);
		hNextObject = GetPredElement (ObjectPtr);
		GetElementStarShip (ObjectPtr, &EnemyStarShipPtr);

		if (!elementsOfSamePlayer (ObjectPtr, ShipPtr)
			&& CollisionPossible (ObjectPtr, ShipPtr)
			&& ObjectPtr->playerNr != NEUTRAL_PLAYER_NUM
			&& !(ObjectPtr->state_flags & PLAYER_SHIP)
			// Don't shoot at enemy lasers! Brute force solution used
			&& !((EnemyStarShipPtr && EnemyStarShipPtr->SpeciesID == SLYLANDRO_ID)
				|| (EnemyStarShipPtr && EnemyStarShipPtr->SpeciesID == ARILOU_ID)
				|| (EnemyStarShipPtr && EnemyStarShipPtr->SpeciesID == CHMMR_ID)
				|| (EnemyStarShipPtr && EnemyStarShipPtr->SpeciesID == CHENJESU_ID
					&& ObjectPtr->preprocess_func == fragment_preprocess)
				|| (EnemyStarShipPtr && EnemyStarShipPtr->SpeciesID == VUX_ID 
					&& ObjectPtr->preprocess_func == 0)
				|| (EnemyStarShipPtr && EnemyStarShipPtr->SpeciesID == MMRNMHRM_ID
					&& ObjectPtr->preprocess_func == 0)
				|| (EnemyStarShipPtr && EnemyStarShipPtr->SpeciesID == EARTHLING_ID
					&& ObjectPtr->preprocess_func == 0)))
		{
		...
		&& DeltaEnergy (ElementPtr, -AUXILIARY_ENERGY_COST))
	{
		...
		StarShipPtr->auxiliary_counter = AUXILIARY_WAIT + 1;
		...
		COUNT ConcernCounter)
{
	...
	lpEvalDesc = &ObjectsOfConcern[ENEMY_SHIP_INDEX];
	{
		...
		if (StarShipPtr->special_counter == 0
				...
				&& !(EnemyStarShipPtr->RaceDescPtr->ship_info.ship_flags & (LIGHT_POINT_DEFENSE | HEAVY_POINT_DEFENSE))
